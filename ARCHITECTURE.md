...
- **Models** — чистые данные (Marker, SessionData, AppSettings, HotkeyBinding)
- **Services** — бизнес-логика без UI (OBS, FFmpeg, Whisper, Excel, Hotkeys, Session, Pipeline)
...
```
TabControl
├── Настройки   — OBS, директория, аудио, транскрипция, горячие клавиши
├── Запись      — таймер, кнопки Bug/Task/Note, список маркеров
├── Экспорт     — список маркеров с чекбоксами, прогресс генерации
└── История     — список прошлых сессий
```
...
Computed: `TimestampFormatted` — форматированная строка для UI.
...
| Директория | OutputDirectory |
| Аудио | PreRecordSeconds (5), PostRecordSeconds (3), AudioCodec ("wav"), AudioBitrate ("192k") |
| Транскрипция | WhisperModel ("large-v3-turbo"), WhisperLanguage ("ru"), WhisperGlossary |
| Горячие клавиши | List<HotkeyBindingData> |
| История | List<SessionHistoryEntry> |

Горячие клавиши по умолчанию: Ctrl+F1 (Bug), Ctrl+F2 (Task), Ctrl+F3 (Note), Ctrl+F9 (Start), Ctrl+F10 (Stop), Ctrl+F11 (Generate), Ctrl+F12 (OpenFolder).
...
```
whisper-cli.exe -m <model> -f <audio> --offset-t <ms> --duration <ms>
                --prompt <glossary> --output-txt -of <outputBase> -l <lang>
```
...
### PathHelper.cs
Определяет пути к внешним инструментам относительно `Assembly.Location`:
...
2. `download-whisper.ps1` — собрать whisper-cli из исходников (CMake + MSVC) или скачать prebuilt
...
- `ApplicationIcon` удалён из csproj из-за отсутствия `Resources/icon.ico`
- Приложение использует дефолтную иконку .NET
...
- [ ] Иконка приложения (icon.ico не создана)
